...
def remove_mathjax_from_html(html_content):
    """
    Remove apenas scripts MathJax antigos/duplicados, mas preserva a configuração principal.
    Mantém o conteúdo matemático em formato correto.
    """
    # Padrões para remover APENAS scripts MathJax antigos/duplicados
    patterns = [
        r'<script[^>]*src="[^"]*mathjax-config\.js"[^>]*></script>',  # Configs externas
        r'<script[^>]*>\s*//<!\[CDATA\[\s*MathJax\.Hub\.Config[^<]*</script>',  # Config antiga
        r'<script[^>]*>\s*MathJax\.(?:Hub\.)?Queue\s*\[[^\]]*\]\s*</script>',  # Queues antigas
        r'<!--\s*MathJax\s*.*?-->',  # Comentários
    ]
    ...
    # NÃO remover a configuração principal do MathJax
...
def clean_pandoc_html_basic(html_content):
    ...
    Remove cabeçalhos, rodapés, mas PRESERVA classes do MathJax.
    """
    ...
    # Padrões para remover - EXCLUINDO scripts e classes do MathJax
    patterns_to_remove = [
        r'<header[^>]*>.*?</header>',
        r'<footer[^>]*>.*?</footer>',
        r'<nav[^>]*>.*?</nav>',
        ...
        r'<div[^>]*id="TOC"[^>]*>.*?</div>',
        # NÃO remover scripts - eles serão incluídos pelo template
        # NÃO remover classes que começam com "math" ou "tex2jax"
        r'<style[^>]*>.*?</style>',
        r'<link[^>]*>',
        r'<meta[^>]*>',
    ]
    ...
    # Remover atributos indesejados, mas PRESERVAR classes importantes
    def preserve_important_classes(match):
        tag = match.group(0)
        # Verificar se tem classes importantes
        if 'class="' in tag:
            # Extrair classes
            class_match = re.search(r'class="([^"]*)"', tag)
            if class_match:
                classes = class_match.group(1).split()
                # Filtrar apenas classes que queremos preservar
                important_classes = [c for c in classes if 
                                    c.startswith('math') or 
                                    c.startswith('tex2jax') or
                                    c in ['img-fluid', 'rounded', 'shadow', 'mb-3', 'mt-4', 'text-primary']]
                
                if important_classes:
                    # Se tem classes importantes, manter apenas elas
                    new_tag = re.sub(r'class="[^"]*"', f'class="{" ".join(important_classes)}"', tag)
                    return new_tag
                else:
                    # Se não tem classes importantes, remover o atributo class
                    return re.sub(r'\sclass="[^"]*"', '', tag)
        return tag
    
    # Aplicar a preservação de classes
    ...
    # Substituições com preservação de classes existentes
    replacements = [
        (r'<img([^>]+)>', lambda m: f'<img{m.group(1)} class="img-fluid rounded shadow mb-3">' if 'class=' not in m.group(1) else f'<img{m.group(1)}>'),
