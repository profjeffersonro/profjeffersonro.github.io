...
# ============================================================================
# FUNÇÕES DE LIMPEZA HTML - VERSÃO HÍBRIDA
# ============================================================================
...
def clean_pandoc_html_basic(html_content):
    """
    Limpeza básica de HTML do Pandoc.
...
def read_html_file(file_path, clean_html=True):
    """
    Lê um arquivo HTML e aplica as correções necessárias.
    Retorna APENAS o conteúdo principal, sem containers extras.
    """
    try:
        ...
        if clean_html and is_pandoc_html(content):
            # Limpeza básica (remove cabeçalhos/rodapés do Pandoc E containers)
            content = clean_pandoc_html_basic(content)
            
            # Corrigir spans de matemática
            content = fix_math_for_github(content)
        
        # Retornar o conteúdo SEM envolver em divs extras
        # O template do site já vai fornecer a estrutura correta
        return content
